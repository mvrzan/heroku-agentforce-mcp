...
if (!ANTHROPIC_API_KEY) {
  throw new Error(`${getCurrentTimestamp()} - ❌ HTTPMCPClient - ANTHROPIC_API_KEY is not set!`);
}

if (!ANTHROPIC_CLAUDE_MODEL) {
  throw new Error(`${getCurrentTimestamp()} - ❌ HTTPMCPClient - ANTHROPIC_CLAUDE_MODEL is not set!`);
}
...
export class HTTPMCPClient {
  ...
  constructor() {
    ...
    this.mcp = new Client({ name: "http-mcp-client", version: "1.0.0" });
  }

  async connectToServer(serverUrl: string) {
    try {
      console.log(`${getCurrentTimestamp()} - 🔗 HTTPMCPClient - Connecting to HTTP server at: ${serverUrl}`);
      ...
      console.log(
        `${getCurrentTimestamp()} - 🔌 HTTPMCPClient - Connected to HTTP server with tools:`,
      ...
      if (resources.resources && resources.resources.length > 0) {
        console.log(
          `${getCurrentTimestamp()} - 🧰 HTTPMCPClient - Available resources:`,
  ...
  async processQuery(query: string) {
    ...
    try {
      console.log(`${getCurrentTimestamp()} - 🔍 HTTPMCPClient - Discovering available prompts...`);

      // Get list of available prompts
      const promptsList = await this.mcp.listPrompts();

      if (promptsList.prompts && promptsList.prompts.length > 0) {
        console.log(`${getCurrentTimestamp()} - ✅ HTTPMCPClient - Found ${promptsList.prompts.length} prompts`);
        ...
        if (weatherPrompt) {
          console.log(`${getCurrentTimestamp()} - 📋 HTTPMCPClient - Found weather prompt: ${weatherPrompt.name}`);
          ...
          if (promptResponse && promptResponse.messages) {
            console.log(
              `${getCurrentTimestamp()} - 📚 HTTPMCPClient - Successfully retrieved prompt with ${
      ...
      console.error(`${getCurrentTimestamp()} - ⚠️ HTTPMCPClient - Failed to load prompt:`, error);
    }
    ...
    if (weatherKeywords.some((keyword) => query.toLowerCase().includes(keyword))) {
      try {
        ...
        if (requestsRealTimeData && hasWeatherTools) {
          console.log(
            `${getCurrentTimestamp()} - 🤔 HTTPMCPClient - The query likely requires a use of the weather tool. Claude will decide what is the appropriate action.`
          );
    ...
    try {
      ...
      for (const content of response.content) {
        if (content.type === "text") {
          ...
          console.log(
            `${getCurrentTimestamp()} - 🤖 HTTPMCPClient - Calling tool ${toolName} with args ${JSON.stringify(
              toolArgs
            )}`
          );
      ...
      console.error(`${getCurrentTimestamp()} - ❌ HTTPMCPClient - Error processing query:`, error);
  ...
  async chatLoop() {
    ...
    try {
      console.log(`\n${getCurrentTimestamp()} - ✅ HTTPMCPClient - HTTP MCP Client Started!`);
      console.log(`${getCurrentTimestamp()} - ✍️ HTTPMCPClient - Type your queries or 'quit' to exit.`);
